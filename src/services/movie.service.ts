import {IMovieInfo, IMovieResponse, IVideoResponse} from "../interfaces";
...
const moviesService = {
    ...
    getMovieById: async (id: number): Promise<IMovieInfo> => {
        try {
    ...
    searchMoviesByArrayGenre: async (arrayId: number[], page: number): Promise<IMovieResponse> => {
        try {
            const response: AxiosResponse = await axiosInstance.get(
                `/discover/movie?with_genres=${arrayId}&page=${page}`
            );
            return response.data;
        } catch (error) {
            console.error('Error occurred while fetching movies by genres:', error);
